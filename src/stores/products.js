...
export const useProductStore = defineStore({
  ...
  actions: {
    ...
    addToCart(productData) {
      ...
      if (existingProduct) {
        existingProduct.quatity += productData.quatity; // Correct property name 'quatity'
      } else {
        this.CartList.push({ ...productData });
      }
    
      // Reset the quantity in productsList to 0
      const productIndex = this.productsList.findIndex((product) => product.id === productData.id);
      if (productIndex !== -1) {
        this.productsList[productIndex].quatity = 0; // Correct property name 'quatity'
      }
    },
